...
namespace DataUtility {
    ...
    (
        ...
        for (size_t i = 0; i < labels.rows; i++) {
            ...
            for (size_t j = 0; j < labels.cols; j++) {
                if (train_label_count.find(static_cast<float>(j)) == train_label_count.end()) {                                                                 // if label not in train set
                    train_label_count[static_cast<float>(j)] = 0;
				}
                if (test_label_count.find(static_cast<float>(j)) == test_label_count.end()) {                                                                   // if label not in test set
                    test_label_count[static_cast<float>(j)] = 0;
                }

                size_t train_label_increment = labels_matrix(i, j) == 0 ? 0 : 1;
                size_t test_label_increment = labels_matrix(i, j) == 0 ? 0 : 1;

                float new_train_label_percent = static_cast<float>(train_label_count[static_cast<float>(j)] + train_label_increment / (train_sample_count + 1));// new count if added to train set
                float new_test_label_percent = static_cast<float>(test_label_count[static_cast<float>(j)] + test_label_increment / (test_sample_count + 1));    // new count if added to test set
            ...
            if (add_to_train) {
                ...
                for (size_t j = 0; j < labels.cols; j++) {
                        if (labels_matrix(i, j) != 0)
                            train_label_count[static_cast<T>(j)]++;                                                                                         // increasing the label count
                }
                ...
                for (size_t j = 0; j < labels.cols; j++) {
                    if (labels_matrix(i, j) != 0)
                        test_label_count[static_cast<T>(j)]++;                                                                                              // increasing the label count
                }
            }
        }
    ...
    (
        ...
        for (auto& [label, idxs] : label_idxs) {
            ...
            size_t test_label_size = static_cast<size_t>(idxs.size() * split_ratio);
